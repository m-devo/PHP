...
if (isset($_GET['id'])) { 
    ...
    try {
        ...
        if (!$pdo instanceof PDO) {
            throw new Exception("Database connection object is not valid.");
        }

        $dbManager = new UserDatabaseManager($pdo);
        
        $userId = (int)$_GET["id"];
        $userData = $dbManager->getUserById($userId);
...
<main class="flex items-center justify-center min-h-screen p-4">
    <div class="w-full max-w-lg bg-white rounded-lg shadow-md p-6 sm:p-8">
        ...
        <h4 class="text-xl font-medium text-center mb-5">
            (<?php echo $title; ?>) <?php echo $userData['firstName']; ?> <?php echo $userData['lastName']; ?>
        </h4>
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <h1 class="text-lg font-medium"><?php echo $userData['firstName']; ?> <?php echo $userData['lastName']; ?></h1>
            </div>

            <div>
                <h1 class="block text-sm font-medium">Address:</h1>
                <h1 class="text-lg font-medium"><?php echo $userData['address']; ?></h1>    
            </div>

            <div>
                <h1 class="block text-sm font-medium">Email:</h1>
                <h1 class="text-lg font-medium"><?php echo $userData['email']; ?></h1>    
            </div>

            <div>
                <h1 class="block text-sm font-medium">Country:</h1>
                <h1 class="text-lg font-medium"><?php echo $userData['country']; ?></h1>
            </div>
            ...
            <div>
                ...
                <h1 class="text-lg font-medium"><?php echo $userData['department']; ?></h1>
            </div>

            <div>
                <h1 class="block text-sm">Sex:</h1>
                <h1 class="text-lg font-medium"><?php echo $userData['sex']; ?></h1>
            </div>
